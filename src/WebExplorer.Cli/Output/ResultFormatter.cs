...
internal static class ResultFormatter
{
    ...
    public static void WriteConsole(SearchResponse response, bool expandUrls)
    {
    ...
    public static void WriteJson(SearchResponse response, TextWriter writer)
    {
    ...
    public static void WriteInstantAnswer(SearchResponse response)
    {
        if (response.InstantAnswer is not null)
        {
            AnsiConsole.MarkupLine($"  [bold yellow]{Markup.Escape(response.InstantAnswer.Text)}[/]");
